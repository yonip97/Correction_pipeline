...
def main():
    ...
    if 'gpt' in args.model_name:
        ...
        generation_kwargs = {'max_new_tokens': args.max_new_tokens, 'temperature': args.temperature,'top_p': args.top_p}
        ...
        for indexes in tqdm(iter_list(list(range(len(all_texts))[len(outputs):]), args.batch_size)):
            ...
            for i in indexes:
                ...
                input = args.prompt + '\n' + 'Document: \n' + text + '\n' + 'Summary: \n' + summary + '\n' + args.past_text_prompt
                batch.append(input)
            batch_outputs = model.call(inputs=batch, generation_kwargs=generation_kwargs, tokenizer_kwargs=tokenizer_kwargs)
            outputs += batch_outputs
            pd.DataFrame.from_dict(
                {'text': all_texts[:len(outputs)], 'model_summary': all_summaries[:len(outputs)], args.output: outputs}).to_csv(
                args.output_path + '.csv')
    else:
