...
def download_qags(data_source: str) -> None:
    ...
    for example in dataset:
        ...
        d = {
            ...
            'label': label,
            'model':'BART' if data_source == 'xsum' else 'BOTTOM-UP'
        }
        qags_data.append(d)
...
def download_summeval():
    ...
    for line in jsonl_file.iter_lines():
        dataset.append(json.loads(line.decode()))
    summeval_data = []
    model_ids_to_models = {'M0':'extractive','M1': 'extractive', 'M2': 'extractive', 'M3': 'extractive', 'M4': 'extractive',
                           'M5': 'extractive',
                           'M6': 'extractive', 'M7': 'extractive',
                           'M8': 'Pointer Generator', 'M9': 'Fast-abs-rl', 'M10': 'Bottom-Up', 'M11': 'Improve-abs',
                           'M12': 'Unified-ext-abs',
                           'M13': 'ROUGESal', 'M14': 'Multi-task (Ent + QG )', 'M15': 'Closed book decoder',
                           'M16': 'SENECA', 'M17': 'T5',
                           'M18': 'NeuralTD', 'M19': 'BertSum-abs', 'M20': 'GPT-2', 'M21': 'UniLM', 'M22': 'BART',
                           'M23': 'PEGASUS'}
    for example in dataset:
        ...
        d = {
            ...
            'model': model_ids_to_models[example['model_id']]
        }
...
def main() -> None:
    ...
    download_fever()
    print(
        'Done downloading and formatting the TRUE datasets. Use the grounding, generated_text and label columns in the resulting .csv files.')
