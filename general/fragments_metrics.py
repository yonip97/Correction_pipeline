...
class Fragments(object):
    ...
    # def start(self, summary, text, tokenize=True, case=False):
    #
    ...
    def score(self, metrics, summaries, texts, tokenize=True, case=False):
        ...
        matches_list = [self._match(summary, text) for summary, text in zip(summaries_norms, texts_norms)]
        results = {}
        if 'coverage' in metrics:
            results['coverage'] = [self.new_coverage(matches=matches, summary=summary) for matches, summary in
                                   zip(matches_list, summaries)]
        if 'density' in metrics:
            results['density'] = [self.new_density(matches=matches, summary=summary) for matches, summary in
                                  zip(matches_list, summaries)]
        if 'compression' in metrics:
            results['compression'] = [self.new_compression(text=text, summary=summary) for text, summary in
                                      zip(texts, summaries)]
        return results
    ...
    def new_coverage(self, matches, summary):

        numerator = sum(o.length for o in matches)
    ...
    def new_density(self, matches, summary):
        numerator = sum(o.length ** 2 for o in matches)
        denominator = len(summary)
    ...
    def new_compression(self, summary, text):
...
class Fragments_from_RL():
    ...
    def score(self, metrics, summaries, texts, tokenize=True, case=False):
        ...
        print("processing summaries")
        #summaries = [x for x in tqdm(self._en.pipe(summaries, disable=["tagger", "parser", "ner", "textcat"]))]
        summaries = [word_tokenize(summary.lower()) if tokenize else summary.split() for summary in summaries]
        print("processing texts")
        #texts = [x for x in tqdm(self._en.pipe(texts, disable=["tagger", "parser", "ner", "textcat"]))]
        texts = [word_tokenize(text.lower()) if tokenize else text.split() for text in texts]
        #summaries_norms = [self._normalize(summary, case) for summary in summaries]
        #texts_norms = [self._normalize(text, case) for text in texts]
        matches_list = [self._match(summary, text) for summary, text in zip(summaries, texts)]
        results = {}
        if 'coverage' in metrics:
            results['coverage'] = [self.new_coverage(matches=matches, summary=summary) for matches, summary in
                                   zip(matches_list, summaries)]
    ...
    def new_density(self, matches, summary):
        numerator = sum(o.size **2 for o in matches)
        denominator = len(summary)
    ...
    def new_coverage(self, matches, summary):
        ...
        numerator = sum(o.size for o in matches)
        denominator = len(summary)
        if denominator == 0:
            return 0
        else:
...
def main():
    texts = ['hello my name is artur', 'hello my name is artur', 'hello my name is artur', 'hello my name is artur']
    summaries = ['hello','hello','hello','hello']
    fragments_metric = Fragments()
    fragments_results = fragments_metric.score(metrics=['density', 'coverage','compression'], texts=texts,
                                                  summaries=summaries)
    for key in fragments_results.keys():
        print(key, np.mean(fragments_results[key]))