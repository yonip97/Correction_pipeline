...
class FormatTransformation(Transformation):
    ...
    def transform(self, examples):
        ...
        summaries = [x['summary'] for x in examples]
        page_docs = [x for x in self.spacy.pipe(texts, disable=["tagger"])]
        summaries = [x for x in self.spacy.pipe(summaries)]
        claims = [x for x in self.spacy.pipe(summaries)]
        for example, page_doc, summary, claim in zip(examples, page_docs, summaries, claims):
