...
def compute_batched_sentence_rouge(article_sents, summary_sents, rouge):
    rouge_scores = {}
    for i, sen in enumerate(summary_sents):
        predictions = [sen] * len(article_sents)
        scores = rouge.compute(predictions=predictions, references=article_sents, use_aggregator=False)
        rouge_scores[i] = {}
        rouge_scores[i]['rouge_scores'] = {}
        for k, v in scores.items():
            indices = np.argsort(v)[::-1]
            rouge_scores[i]['rouge_scores'][k] = indices.tolist()
    return rouge_scores
    ...
    def __getitem__(self, idx):
        ...
        if self.args.use_entire_summaries:
            ...
            if self.args.use_entire_article_as_source:
                ...
                relevant_sents = [" ".join(entry["source_article_sentences"][idx - 2:idx + 2]) for idx in
                                  relevant_indices]
            ...
            if self.args.add_full_summary_in_context:
                input = gen_summ_sent + " <sep> " + " ".join(
                    entry["generated_summary_sentences"]) + " <sep> " + source_article
            else:
        ...
        return torch.tensor(model_inputs["input_ids"]), torch.tensor(model_inputs["labels"]), torch.tensor(
            idx), source_article
    ...
    def process_ood_test_example(self, entry, detect_preds=None, detect=False):
        ...
        if self.args.use_entire_summaries:
            ...
            if self.args.detect_and_correct and detect:
                ...
                pass
            model_inputs = self.tokenizer(input, max_length=self.args.max_input_len, padding=self.padding,
                                          truncation=True)
            ...
            if self.args.use_relevant_sents_as_source:
                rouge_scores = compute_batched_sentence_rouge(entry["source_article_sentences"],
                                                              entry["generated_summary_sentences"], self.rouge)
            for sent_idx, gen_summ_sent in enumerate(entry["generated_summary_sentences"]):
                ...
                if self.args.use_entire_article_as_source:
                    ...
                    relevant_article_sent_indices = sent_summ_art_rouge["rougeLsum"][0:3]
...
class Summarizer(pl.LightningModule):
    ...
    def __init__(self, params):
        ...
        if self.args.resume_checkpoint_dir != "None":
            ...
            self.model = AutoModelForSeq2SeqLM.from_pretrained(None, config=config,
    ...
    def _evaluation_step(self, split, batch, batch_nb):
        ...
        for metric_name in bert_metric_names:
            metric_val = input_ids.new_zeros(1) + sum(bertscore_results[metric_name]) / len(
    ...
    def _evaluation_epoch_end(self, split, step_outputs):
        metric_names = ['rouge1', 'rouge2', 'rougeL', 'rougeLsum', 'bertscore_precision', 'bertscore_recall',
                        'bertscore_f1', 'fact_label_f1']
        aggregated_metrics = {}
        for metric_name in metric_names:
    ...
    def add_model_specific_args(parser):
        ...
        parser.add_argument("--use_sentence_level_summaries", type=bool, default=True,
                            help="Use single sentence of summary for correction")
        # changed from False
        parser.add_argument("--use_entire_article_as_source", type=bool, default=True,
                            help="Use entire document as context")
        # changed from False
        parser.add_argument("--use_relevant_sents_as_source", type=bool, default=True,
                            help="Use only relevant sents to summary sents from article as context")
        # changed from False
        parser.add_argument("--add_full_summary_in_context", type=bool, default=True,
        ...
        parser.add_argument("--transformer_model", type=str, default='facebook/bart-base',
                            help="transformer_model tyoe")
        ...
        parser.add_argument("--ood_test_datapath", type=str, default='./data/bart_test.json',
                            help="Location of input dir")
        parser.add_argument("--baseline_fcc_file", type=str,
                            default='/remote/bones/user/vbalacha/summary_fact_corrector/output_dir/bart_large_cnn/factcc_corr/factcc_eval_predictions_wcnnid.jsonl',
                            help="Location of FactCC Results of Baseline")
        ...
        parser.add_argument("--do_predict", type=bool, default=False,
                            help="Do Predict")
        # changed from False
        parser.add_argument("--do_predict_ood", type=bool, default=True,
                            help="Do Predict OOD")
...
if __name__ == "__main__":
    ...
    summarizer.hf_dataset = {"test": BERTS2S_TConvS2S_xsum_trained_dataset()}
    ...
    if args.do_predict_ood:
        ...
        summarizer.hf_dataset = {"ood_test": BERTS2S_TConvS2S_xsum_trained_dataset()}
        dataset_split = summarizer.hf_dataset['ood_test']
        orig_test_dataset = SummarizationDataset(hf_arxiv_dataset=dataset_split, tokenizer=summarizer.tokenizer,
                                                 args=summarizer.args)
        ...
        with open(output_test_preds_file, "w") as writer, open(fcc_test_file, "w") as fccp, open(dae_test_file,
                                                                                                 "w") as daep:
            for idx, entry in tqdm(enumerate(dataset_split)):
                entry['source_article_sentences'] = nltk.sent_tokenize(entry['text'])
                entry['generated_summary_sentences'] = nltk.sent_tokenize(entry['summary'])
                # if idx >= 10:
                #     break
                source_article_sentences = entry["source_article_sentences"]
                if len(source_article_sentences) == 0:
                    ...
                    continue
                source_article = " ".join(source_article_sentences)
                source_summary = " ".join(entry["generated_summary_sentences"])
                predictions = None
                detect_predictions = None
                if args.detect_and_correct:
                    gensumm_sent_input_ids, gen_summ = orig_test_dataset.process_ood_test_example(
                        entry, detect=True)
                    ...
                    outputs = summarizer.model.generate(input_ids=gensumm_sent_input_ids,
                                                        attention_mask=(
                                                                    gensumm_sent_input_ids != summarizer.tokenizer.pad_token_id),
                                                        use_cache=False, max_length=args.max_output_len,
                                                        num_beams=args.num_beams,
                                                        # use_cache=False, max_length=args.max_output_len, num_beams=5,
                                                        return_dict_in_generate=True, output_hidden_states=True)
                    generated_ids = outputs["sequences"]
                    detect_predictions = summarizer.tokenizer.batch_decode(generated_ids.tolist(),
                                                                           skip_special_tokens=True)

                    gensumm_sent_input_ids, gen_summ = orig_test_dataset.process_ood_test_example(
                        entry, detect_preds=detect_predictions, detect=False)
                ...
                if args.generate_factuality_label:
                    ...
                    for pred in predictions:
                        if "<sep>" in pred and (
                                pred.split("<sep>")[0].strip() == '0' or pred.split("<sep>")[0].strip() == '1'):
                            pred_label, pred_text = pred.split("<sep>")
                ...
                print(preds)
                corrected_summary = " ".join(preds)
                final_results.append((source_article,corrected_summary))
        import pandas as pd
        pd.DataFrame.from_records(final_results,columns=['source','summary']).to_csv('data/factedit/poc_data_corrected.csv')
                ...
                #                           "ref": original_summary, "old_summ": gen_summ}
            ...
            #     if args.use_sentence_level_summaries:
            #         aggregated_metrics[f'{split}_fact_summ_incorrect'] = sum(
            #             aggregated_metrics[f'{split}_fact_summ_incorrect']) / len(
            #             aggregated_metrics[f'{split}_fact_summ_incorrect'])
            #
            ...
            # fp.write(json.dumps(aggregated_metrics))
            # fp.close()
            # copyfile(os.path.join(args.output_dir, 'metrics.json'),
            #          os.path.join(args.output_dir, 'ood_test_metrics.json'))
'''
