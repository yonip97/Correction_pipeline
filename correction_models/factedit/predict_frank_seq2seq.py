...
from correction_models.factedit.train_seq2seq import Summarizer, SummarizationDataset
...
if __name__ == "__main__":
    ...
    summarizer.hf_dataset = datasets.load_dataset('json', data_files={"ood_test": "data/frank_raw/benchmark_data.json"})
    ckpt_path = None
    ...
    if args.do_predict_ood:
        ...
        for idx, entry in tqdm(enumerate(dataset_split)):
            ...
            gensumm_sent_input_ids, gen_summ = orig_test_dataset.process_ood_test_example(entry)
        ...
        predictions = [x['predictions']['pred'] for x in result_aggregator]
        old_summaries = [x['predictions']['old_summ'] for x in result_aggregator]
        source_articles = [x['predictions']['source_article'] for x in result_aggregator]
        models = [x['predictions']['model_name'] for x in result_aggregator]
        pd.DataFrame(data={"predictions": predictions, "old_summaries": old_summaries,
                           "source_articles": source_articles,"models":models}).to_csv('data/factedit/frank_cnn_model.csv')
            ...
            #                     if args.use_sentence_level_summaries:
            #                         aggregated_metrics[dataset][data_split][model_name][
            #                             f'{split}_fact_summ_incorrect'] = []
            #
            # predictions = []
            ...
            #                 aggregated_metrics[dataset][data_split][model_name][f'{split}_{key}'] = sum(value) / len(
            #                     value)
            #
            #                 if args.generate_factuality_label:
            #                     aggregated_metrics[dataset][data_split][model_name][f'{split}_fact_incorrect'] = sum(
            #                         aggregated_metrics[dataset][data_split][model_name][f'{split}_fact_incorrect'])
            #                     aggregated_metrics[dataset][data_split][model_name][f'{split}_fact_correct'] = sum(
            #                         aggregated_metrics[dataset][data_split][model_name][f'{split}_fact_correct'])
            #                     aggregated_metrics[dataset][data_split][model_name][f'{split}_fact_num'] = sum(
            #                         aggregated_metrics[dataset][data_split][model_name][f'{split}_fact_num'])
            #                     if args.use_sentence_level_summaries:
            #                         aggregated_metrics[dataset][data_split][model_name][
            #                             f'{split}_fact_summ_incorrect'] = sum(
            #                             aggregated_metrics[dataset][data_split][model_name][
            #                                 f'{split}_fact_summ_incorrect']) / len(
            #                             aggregated_metrics[dataset][data_split][model_name][
            #                                 f'{split}_fact_summ_incorrect'])
            #
