...
from transformers import T5ForConditionalGeneration, T5Tokenizer, AutoTokenizer, AutoModelForCausalLM,BitsAndBytesConfig
...
def llm_revision():
    ...
    revision_model = AutoModelForCausalLM.from_pretrained('meta-llama/Meta-Llama-3-70B-Instruct',device_map='auto',
                                                          quantization_config=bnb_config)


    revision_tokenizer = AutoTokenizer.from_pretrained('meta-llama/Meta-Llama-3-70B-Instruct')
    ...
    for text, summary in tqdm(zip(texts, summaries)):
        ...
        s  = time.time()
        model_input = [(prompt + '\n' + 'Summary: \n' + summary + '\n', "Text: \n" + text)]
        model_input = revision_tokenizer.batch_encode_plus(model_input, max_length=1024, return_tensors='pt',
                                                           padding=False,
                                                           truncation='only_second').to('cuda')
        revised_summary = revision_model.generate(**model_input)
        counter += 1
        print(f"Time for one revision: {time.time() - s}")
        if counter == 100:
            break
