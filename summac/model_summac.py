...
def batcher(iterator, batch_size=4, progress=True):
    if progress:
...
class SummaCImager:
    ...
    def build_image(self, original, generated):
        ...
        for batch in batcher(dataset, batch_size=20, progress=False):
...
class SummaCConv(torch.nn.Module):
    ...
    def forward(self, originals, generateds, images=None):
        if images is not None:
            ...
            images, histograms = [], []
            for original, generated in tqdm(zip(originals, generateds)):
...
class SummaCZS:
    ...
    def score(self, originals, generateds, batch_size=128, **kwargs):
        images = self.imager.build_images(originals, generateds, batch_size=batch_size)
